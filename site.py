...
oauth = OAuth()
#connection = pymysql.connect(host='localhost',user='root',password='password123',db='sentiment_analysis', charset='utf8mb4',cursorclass=pymysql.cursors.DictCursor)
UPLOAD_FOLDER = 'FilesUploading'
...
def getsentiments():
    try:
        ...
        if request.method == 'POST':
            ...
            word_freqs_js, max_freq, wcfilename = wordCloudCaller(val)
            print("words",word_freqs_js)
            print("Max Freq",max_freq)
            print("wcfilename",wcfilename)
            results.append(wordfrquencies)
            results.append(allsummary)
            results.append(wcfilename)
            
        return render_template('result-new.html',results = results, word_freqs=word_freqs_js, max_freq=max_freq, wcfilename=wcfilename)
...
def wordCloudCaller(text):
    ...
    wcfilename = ''
    import random
    wcfilename = "static\\" + wcfilename + str(random.randint(0,10000)) + '.png'
    wc = WordCloud(stopwords = stopword1).generate(text)
    plt.imshow(wc)
    plt.figure(1,figsize=(5,5))
    plt.axis('off')
    plt.title('Wordcloud')
    plt.savefig(wcfilename)
    plt.close()
    return word_freqs_js, max_freq, wcfilename
